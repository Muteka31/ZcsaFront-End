...
export default function Category(){
    ...
    const categories = useFetch(BASE_URL+'/v1/get/category');
    console.log(categories);
    const category_list = [];


    const enableCategory = () => {
        axios.post(BASE_URL+'/v1/enable/category',{
            "id":categoryId
        })
            .then(response => {
    ...
    const disableCategory = () => {
        axios.post(BASE_URL+'/v1/disable/category',{
            "id":categoryId
        })
            .then(response => {
                console.log("Status: ", response.status);
    ...
    categories.map((category) => (
        category_list.push({
            id:category.id,
            name:<div><b>{category.category}</b></div>,
            status:<div>
                <MDBBadge color="success" className="p-1">{ category.status ? 'Active' : null}</MDBBadge>
                <MDBBadge color='danger'>{ !category.status ? 'Disabled' : null}</MDBBadge>
            </div>,
            action:<div>
                ...
                { category.status ?
                    <MDBBtn className="mt-0 btn btn-sm btn-dark" onClick={() =>{
                        setCategoryId(category.id);
                        setCategoryName(category.catergory);
                        setDisableShow(true);
    ...
    const createCategory = (event) => {
        ...
        axios.post(BASE_URL+'/v1/category',{
            //"id":provinceId,
        ...
        <>
            ...
            <MDBContainer>
                <MDBBreadcrumb color="warning-color">
                    ...
                    <MDBBreadcrumbItem active><b>Category Management </b></MDBBreadcrumbItem>
                </MDBBreadcrumb>
                <MDBRow>
                    <MDBCol md='12'>
                        <MDBCard className="border border-light">
                            <MDBCardHeader color='special-color'>Category List</MDBCardHeader>
                                ...
                                <form onSubmit={createCategory}>
                                    <MDBRow className="">
                                        <MDBCol style={{marginTop: '-1.1rem'}} md="11">
                                            <MDBInput style={{paddingTop: '1rem'}} id="categoryName" labelClass="labelBg"
                                                      outline type='text' label='Enter Category Name: '
                ...
                >
                    <Modal.Header closeButton>
                        ...
                        <MDBBtn className="mt-0 btn btn-block btn-sm btn-dark" onClick={() =>{ disableCategory()}}><b>Disable Category {categoryName}<MDBIcon icon="ban" className="ml-2"/></b></MDBBtn>
                        {/* </MDBAnimation> */}
                    </Modal.Header>
                </Modal>
                <Modal
                ...
                >
                    <Modal.Header closeButton>
                        {/* <MDBAnimation type="pulse" infinite> */}
                        <Button variant="success" className="mt-0 btn btn-block btn-sm btn-success" onClick={() =>{ enableCategory()}} ><b>Enable Category {categoryName}<MDBIcon icon="check" className="ml-2"/></b></Button>
                        {/* </MDBAnimation> */}
                    </Modal.Header>
                </Modal>
                ...
                >
                    ...
                    <Modal.Body closeButton>
                        <MDBCard>
                            <MDBCardHeader color='special-color'>Category: {categoryName} </MDBCardHeader>
                            <MDBCardBody>
                                <form onSubmit={updateContainer}>
                                    <MDBRow className="">
                                        <MDBCol md="11">
                                            <MDBInput id="categoryName" labelClass="labelBg"  outline type='text' label='Enter Category Name: '
                                                      value={categoryName} onChange={(e) => setCategoryName(e.target.value)}/>
                                        </MDBCol>
                                        <MDBCol md="11">
                                            <div style={{marginTop: '1.1rem'}}></div>
